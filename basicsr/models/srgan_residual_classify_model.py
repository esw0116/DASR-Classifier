...
class SRGANReidualClassifyModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRGANReidualClassifyModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        sr_output_ds = F.interpolate(sr_output, size=(lr_h, lr_w), mode='bicubic')
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            sr_output = self.net_g(self.lq.contiguous(), weights)
        lr_h, lr_w = self.lq.shape[-2:]
        self.sr_output_ds = F.interpolate(sr_output, size=(lr_h, lr_w), mode='bicubic')
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                if self.opt['is_train']:
                    ...
                    save_img_path_sr = osp.join(self.opt['path']['visualization'], dataset_name, f'{current_iter}', 'sr',f'{img_name}.png')
                    ...
                    if self.opt['val']['suffix']:
                        ...
                        save_img_path_sr = osp.join(self.opt['path']['visualization']+'_sr', dataset_name,
                                                 f'{img_name}_{self.opt["val"]["suffix"]}.png')
                    else:
                        save_img_path_rd = osp.join(self.opt['path']['visualization']+'_residual', dataset_name,
