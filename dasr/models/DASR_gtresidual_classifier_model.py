...
from basicsr.models.srgan_gtresidual_classify_model import SRGANGTReidualClassifyModel
...
class DASRGTResidualClassifierModel(SRGANGTReidualClassifyModel):

    def __init__(self, opt):
        super(DASRGTResidualClassifierModel, self).__init__(opt)
        self.jpeger = DiffJPEG(differentiable=False).cuda()
        self.usm_sharpener = USMSharp().cuda()
        self.queue_size = opt['queue_size']
        self.resize_mode_list = ['bicubic', 'bicubic', 'bicubic']
    ...
    def update_temperature(self):
        self.net_c.module.update_temperature()
    ...
    def feed_data(self, data_all):
        if self.is_train:
            ...
            if self.degradation_degree == 'severe_degrade_two_stage':
                ...
                self.degradation_params = torch.zeros(self.opt_train['batch_size_per_gpu'], self.num_degradation_params)  # [B, 33]
                self.degradation_classes = torch.zeros(self.opt_train['batch_size_per_gpu']).long()  # [B] 0~3, 0~2, 0~1 from 000 to 321
                ...
                self.degradation_params = self.degradation_params.to(self.device)
                self.degradation_classes = self.degradation_classes.to(self.device)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(DASRGTResidualClassifierModel, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
